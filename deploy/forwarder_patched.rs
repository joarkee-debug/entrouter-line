...
impl Forwarder {
    ...
    pub async fn send_to_node(
        ...
        // Fragment data to fit within MAX_PAYLOAD after relay header + encryption
        let header_overhead = 4 + 1 + dest_node.len(); // flow_id(4) + dest_len(1) + dest_node
        let max_data = wire::MAX_PAYLOAD.saturating_sub(header_overhead + wire::AUTH_TAG_SIZE);

        for chunk in data.chunks(max_data.max(1)) {
            let payload = encode_relay_header(flow_id, dest_node, chunk);
